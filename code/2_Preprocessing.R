...
stomach       <- data_to_sparse_matrix("data/GSM5515944_Stomach.st.txt")
colon         <- data_to_sparse_matrix("data/GSM5515944_Colon.st.txt")
SI            <- data_to_sparse_matrix("data/GSM5515944_SI.st.txt")
Spleen        <- data_to_sparse_matrix("data/GSM5515944_Spleen.st.txt")
blood         <- data_to_sparse_matrix("data/GSM5515945_blood.st.txt")
bloodCR       <- data_to_sparse_matrix("data/GSM5515946_bloodCR.st.txt")
bonemarrow    <- data_to_sparse_matrix("data/GSM5515945_bonemarrow.st.txt")
bonemarrowCR  <- data_to_sparse_matrix("data/GSM5515946_bonemarrowCR.st.txt")
colonCR       <- data_to_sparse_matrix("data/GSM5515946_colonCR.st.txt")
stomachHP     <- data_to_sparse_matrix("data/GSM5515945_stomachHP.st.txt")
...
# eosinophil_allsamples <- merge(blood, c(bonemarrow,  colon, SI, Spleen, stomach), 
#                                add.cell.ids = 
#                                    c("blood",   "bonemarrow",   "colon", "smallintestine", "spleen", "stomach")
# )
...
p = DimPlot(eosinophil_allsamples, order=T, group.by = "seurat_clusters", pt.size = 0.1, label=T, cols = col_vector) 
ggsave(plot =p, filename = here("Figures/allsampleUMAP.pdf"), width = 6, height = 5) 
...
ggsave(filename = here("Figures/allsampleDimPlot.pdf"), plot = a + theme_void(), width = 7, height = 4) 

eos.loom = as.loom(eosinophil_allsamples, filename = "eos_allsamples.loom", 
                   verbose = FALSE)
eos.loom$close_all()
# l1_file = here("eos_allsamples.loom")
